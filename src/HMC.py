...
if __name__ == "__main__":
    ...
    batch_y0 = tf.cast(batch_y0, tf.float32)
    batch_yN = tf.cast(batch_yN, tf.float32)
    ...
    class ODEModel(tf.keras.Model):
        ...
        def call(self, inputs, **kwargs):
            ...
            h_out = tf.concat(
                [r1 / sigma_normal1, r2 / sigma_normal2, r3 / sigma_normal3, r4 / sigma_normal4, r5 / sigma_normal5,
                 r6 / sigma_normal6, r7 / sigma_normal7], 1)
            return h_out
    ...
    def kinetic_energy(V, loggamma_v, loglambda_v):
        q = (tf.reduce_sum(-V ** 2) - loggamma_v ** 2 - loglambda_v ** 2) / 2.0
        return q


    @tf.function
    def compute_gradient_param(dWeights, loggamma, loglambda, batch_size, para_num):
        WW = model.trainable_weights[0]
        dWeights = tf.exp(loggamma) / 2.0 * dWeights + tf.exp(loglambda) * tf.sign(WW)
        return dWeights


    @tf.function
    def compute_gradient_hyper(loss, weights, loggamma, loglambda, batch_size, para_num):
        grad_loggamma = tf.exp(loggamma) * (loss / 2.0 + 1.0) - (batch_size / 2.0 + 1.0)
        grad_loglambda = tf.exp(loglambda) * (tf.reduce_sum(tf.abs(weights)) + 1.0) - (para_num + 1.0)
    ...
    def compute_Hamiltonian(loss, weights, loggamma, loglambda, batch_size, para_num):
        H = tf.exp(loggamma) * (loss / 2.0 + 1.0) + tf.exp(loglambda) * (tf.reduce_sum(tf.abs(weights)) + 1.0) \
    ...
    def leap_frog(v_in, w_in, loggamma_in, loglambda_in, loggamma_v_in, loglambda_v_in):
        ...
        for m in range(L):
            ...
            dWeights = dWeights[0]  # make the gradient to be numpy array
            dWeights = compute_gradient_param(dWeights, loggamma_new, loglambda_new, batch_size, para_num)
            grad_loggamma, grad_loglambda = compute_gradient_hyper(loss, w_new, loggamma_new, loglambda_new, batch_size,
                                                                   para_num)
            ...
            w_new = model.trainable_weights[0] + epsilon * v_new
    ...
    epsilon = tf.constant(0.001, dtype=tf.float32)  # leap frog step size
    epsilon_max = tf.constant(0.0002, dtype=tf.float32)  # max 0.001
    epsilon_min = tf.constant(0.0002, dtype=tf.float32)  # max 0.001


    def compute_epsilon(step):
        coefficient = tf.math.log(epsilon_max / epsilon_min)
        return epsilon_max * tf.exp(- step * coefficient / niters)
    ...
    for step in range(niters):
        ...
        print(step)
        v_initial = tf.random.normal((para_num, 1))  # initialize the velocity
        loggamma_v_initial = tf.random.normal((1,))
        loglambda_v_initial = tf.random.normal((1,))
        ...
        p_temp = np.exp(-loss_finial + loss_initial + \
                        kinetic_energy(v_new, loggamma_v_new, loglambda_v_new) - kinetic_energy(v_initial,
                                                                                                loggamma_v_initial,
                                                                                                loglambda_v_initial))
